...
export class SocialNetworkService implements SocialNetworkServiceModel {
  ...
  async findOne(uuid: string): Promise<Either<Exception, SocialNetworkModel>> {
    const socialNetwork = await this.apiService.get<SocialNetworkProps>('social-network', {
      params: [uuid]
    })
    return socialNetwork.map((data) => SocialNetwork.create(data))
  }
  ...
  async getAllTypeSocialNetworks(): Promise<Either<Exception, string[]>> {
    const socialNetwork = await this.apiService.get<{ types: string[] }>('social-network', {
      params: ['all-type-social-networks']
    })
    return socialNetwork.map((data) => data.types)
  }

  async create(options: CreateSocialNetworkDto): Promise<Either<Exception, SocialNetworkModel>> {
    const socialNetwork = await this.apiService.post<CreateSocialNetworkDto, SocialNetworkProps>(
      'social-network',
      options
    )
    return socialNetwork.map((data) => SocialNetwork.create(data))
  ...
  async updateOne(
    ...
    const socialNetwork = await this.apiService.patch<UpdateSocialNetworkDto, SocialNetworkProps>(
      'social-network',
      options,
      { params: [uuid] }
    )
  ...
  async removeOne(uuid: string): Promise<Either<Exception, SocialNetworkModel>> {
    const socialNetwork = await this.apiService.delete<SocialNetworkProps>('social-network', {
      params: [uuid]
    })
    return socialNetwork.map((data) => SocialNetwork.create(data))
  }
